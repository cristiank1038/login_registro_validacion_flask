...
class User:
    ...
    def save(cls, formulario):
        ...
        result = connectToMySQL('login_registro_validacion_flask').query_db(query, formulario) #1 - Insert recibe id
        return result #result = Identificador del nuevo registro

    @staticmethod   #no usa nada de clase y nada de estancia
    def valida_usuario(formulario):
        ...
        results = connectToMySQL('login_registro_validacion_flask').query_db(query, formulario)
        if len(results) >= 1:
    ...
    def get_by_email(cls, formulario):
        ...
        #}
        query = "SELECT * FROM users WHERE email = %(email)s"
        result = connectToMySQL('login_registro_validacion_flask').query_db(query, formulario)
        if len(result) < 1:
            return False
        else:
    ...
    def get_by_id(cls, formulario):
        ...
        result = connectToMySQL('login_registro_validacion_flask').query_db(query, formulario) #Select recibe lista
        user = cls(result[0])
        return user